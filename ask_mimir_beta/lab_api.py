...
from platform_intent import (
    ...
    platform_award_follow_up_intent,
...
Lead with a compact table containing program, forward assessment, observed FY2023-FY2025 activity
and the main reason. Do not print internal scores, calculation versions or references to a frozen
ranking. If a recent authoritative event changes the conclusion from the completed-year evidence,
explain the change in ordinary customer-facing language.
...
Never add those values together. If discretionary and mandatory figures are reported for the same
fiscal year, label them as separate funding proposals and do not imply that one replaces, includes
or should be added to the other unless the underlying budget source says so. State current and
comparison values whenever describing growth. A program with missing mapped obligations must be
described as not observed in that measure, not as having zero activity.

When annual reported subcontract value or supplier counts fall while prime obligations rise, do not
infer that production work or supplier participation necessarily fell. The subcontract series records
reports dated in each fiscal year; an earlier reported subcontract can support work in later years.
Describe the divergence and the reporting basis briefly. Name material disagreements between measures,
such as rising obligations alongside a falling discretionary request.
...
Finish with a short "Evidence used" section. Explain the scoring methodology only when the user asks
how a ranking was calculated. Keep the answer below 1,200 words.
""".strip()
...
and the action descriptions only when explaining modifications. Call the financial measure net prime
obligations and state its exact fiscal-year period; never call it lifetime value, revenue, contract ceiling
or total program value. Show only fiscal years containing reported contract actions unless the user
explicitly asks for the complete annual series. Do not discuss pre-award years merely because they are
available in the underlying data. FY2026 is partial.

Present prime obligations and Mimir-modelled reported subcontract value as separately labelled measures
without adding a routine paragraph explaining that separation. When reported subawards exist, identify the material supplier sites, descriptions and
...
For an opportunity or an explicit competition question, lead with historical awards that directly
name the program; these are the strongest incumbent evidence available. Keep broader category and
buyer comparisons in a separate wider-market section. For a completed-contract overview, omit the
competitor comparison unless the user explicitly asks about competition, alternatives or likely bidders.
...
short analytical sections. Generic source descriptions such as "see below" or "see complete
description" are not useful capability descriptions and should not be repeated. When
same_platform_reported_descriptions are supplied, use them only as broader evidence about that
supplier site's work on the same platform, not as the exact scope of this subaward. Authoritative
government or first-party sources may be used to clarify a material supplier role. When a reported
description is only "material", "service" or a similarly generic term, use broader_site_capability_evidence
and authoritative web evidence to provide useful site context. Label that context as the site's broader
capability rather than the exact subaward scope. Do not write a paragraph cataloguing details that the
generic description does not establish.

Do not flag geography merely because a registered supplier location differs from the prime award's
place of performance, or because a reported subaward location is blank. Mention a location issue only
when the evidence explicitly identifies a conflicting or invalid field that changes the conclusion.

Finish with an Evidence used section and mention the downloadable CSV evidence pack. Keep the
standard answer below 1,200 words.
...
Use the preceding conversation to understand follow-ups. If the user supplies the platform after a
clarification, answer the outstanding question; do not ask them to choose a topic again. Prior assistant
claims provide conversational context, not independently verified evidence.

Answer for the resolved platform or program. Follow requested_answer_mode exactly:
- platform_awards: lead with the major related awards, modernization work, recipients, scope,
  announced values, funded amounts where available, and performance periods. Answer an ongoing-awards
  question as active awarded work; discuss solicitations only if requested. Keep ceilings, obligations
  and program budgets separate and do not substitute a general supplier overview.
...
- supplier_overview: identify the established platform prime or system integrator separately, then
  ...
  recipients as though they were the supply chain.
- supplier_roles: answer only what the principal suppliers provide. Distinguish platform primes and
  integrators, reported first-tier supplier sites, and wider item-level suppliers. Lead with a table containing
  supplier, CAGE and location, capability or item, relationship route and supporting award examples.
  Do not add momentum rankings, budget comparisons, production forecasts or a general platform brief.
- supplier_value_ranking: mention the established prime or integrator in one short sentence, then rank
  reported first-tier supplier sites by Mimir-modelled reported subcontract value. The table must contain rank,
  supplier, CAGE and location, value and fiscal-year period, capability or item, and reported customer
...
For a broad "everything" or full supply-chain overview, cover the material current modernization
and sustainment awards that establish major supplier roles. Check authoritative current sources for
the principal integration and subsystem awards, their announced values, dates and performance periods.
Do not let a small historical award sample or incidental spare part dominate the commercial picture.
Keep announced award values and ceilings separate from actual obligations and program budgets.
For a follow-up about larger or ongoing awards, lead with those awards and supplier roles, not a new
general overview or a question about whether the user means contracts or solicitations.
...
When coverage.reported_supplier_lane_is_sparse is true, do not use the reported supplier rows as
an overview of the complete supplier base. State the reported first-tier count and value briefly,
then use the NIIN count, DLA recipient-site count and authoritative web sources to describe the wider
industrial footprint. For Stryker and similar cases, a small reported first-tier lane alongside
thousands of associated NIINs is a reporting-coverage finding, not evidence of a small supply chain.
...
award means the site received a directly relevant government award. Identify a manufacturer, repair
provider, distributor, supply-chain provider or integrator only when item evidence or an authoritative
first-party or government source supports that role. An observed procurement recipient must not
automatically be described as the manufacturer.
...
identify substantial work performed in the state when it adds useful context. Do not discuss parent
resolution mechanics, explain how multiple CAGEs should be combined, or recite routine caveats. Keep
...
TOOLS = [
    ...
    {
        "type": "function",
        "name": "search_capability_markets",
        "description": "Resolve a supported market-wide defense capability search to its evidence definition.",
        "strict": True,
        "parameters": {
            "type": "object",
    ...
    {
        "type": "function",
        "name": "get_capability_market",
        "description": "Return CAGE-site suppliers supported by exact item, DLA source, procurement, platform and location evidence for a defined capability.",
        "strict": True,
        "parameters": {
            "type": "object",
            "properties": {
    ...
    {
        "type": "function",
        "name": "get_state_industrial_base",
        "description": "Return time-bounded facility, prime, DLA, reported subcontract and place-of-performance evidence for one US state.",
        "strict": True,
        "parameters": {
            "type": "object",
            "properties": {
...
def is_platform_confirmation(text: str, platform: str) -> bool:
    confirmation = re.sub(
        r"\b(?:as (?:mentioned|discussed|above|before)|as i (?:said|mentioned)|i (?:mean|meant)|yes|the)\b",
        "", str(text or ""), flags=re.IGNORECASE,
    )
    return re.sub(r"[^A-Z0-9]", "", confirmation.upper()) == re.sub(r"[^A-Z0-9]", "", platform.upper())
...
def explicit_platform_query(
    ...
    if len(mentions) == 1 and is_platform_confirmation(text, mentions[0]):
        return mentions[0]
...
def explicit_company_name_query(messages: List[ChatMessage]) -> str | None:
    text = str(messages[-1].content or "").strip()
    if platform_award_follow_up_intent(text):
        return None
...
class LabRuntime:
    def __init__(self) -> None:
        ...
        self.capability_discovery = CapabilityDiscoveryStore(
            data_root,
            precomputed_dir=Path(
                os.getenv(
                    "ASK_MIMIR_CAPABILITY_DIR",
                    str(ROOT / "validation-output" / "capability-markets"),
                )
            ),
        )
        ...
        if self.market_segments.precomputed_dir is not None:
            ...
            if market_segment_manifest.exists():
                release_sources.add(market_segment_manifest)
        if self.capability_discovery.precomputed_dir is not None:
            capability_manifest = (
                self.capability_discovery.precomputed_dir / "manifest.json"
            )
    ...
    def _call_tool(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif name == "search_capability_markets":
            return self.capability_discovery.search(**arguments)
        elif name == "get_capability_market":
...
def sanitize_answer_text(answer: str) -> str:
    ...
    blocked_line_patterns = (
        ...
        r"^\s*Source-reported .+ resolved to .+ parent data",
        r"^\s*Multiple CAGEs? .+ (?:should|must) be treated",
        r"^\s*(?:This|These|The figures?|The values?) .+ not (?:company|supplier) revenue",
        r"^\s*No\s+(?:DIRECT_PROGRAM_HISTORY|direct program history)\b",
        r"^\s*\\?\*?Site fields .+geograph(?:y|ic).+quality",
        r"^\s*FY20\d{2}.+loaded but not observed",
    )
    lines = [
        line
    ...
    cleaned = re.sub(
        r"\s*Multiple CAGEs? for one corporate family[^.]*\.",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
        r"\s*Source-reported [^.]+ resolved to [^.]+ in (?:the )?(?:available|current) parent data\.?",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    cleaned = re.sub(
        r"\bloaded[- ]period total\b",
        "Observed-period total",
        cleaned,
        flags=re.IGNORECASE,
    )
    cleaned = re.sub(
    ...
    cleaned = re.sub(
        r"\s*Keep these figures separate from prime obligations\.[^.]*\.",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
...
def generate_answer(
    ...
    if (
        ...
        state_code = request.active_scope.scope_id.upper()
    if state_code and (
        ...
        emit_progress(
            ...
            "Ranking facilities, contract activity and work performed in the state",
            42,
        )
        pack = runtime.call_tool("get_state_industrial_base", arguments)
        trace = [
        ...
        response = client.responses.create(
            model=runtime.model,
            instructions=STATE_INDUSTRIAL_BASE_PROMPT,
            input=state_input,
            tools=[{"type": "web_search", "search_context_size": "low"}],
            reasoning={"effort": runtime.reasoning_effort},
            max_output_tokens=min(runtime.max_output_tokens, 12000),
        ...
        result = {
            ...
            "model": runtime.model,
            "release_id": runtime.store.manifest["release_id"],
            "answer_artifacts": {"state_industrial_base": pack},
            "tool_trace": trace,
            "latency_ms": round((time.perf_counter() - started) * 1000, 1),
            "response_calls": len(call_usages),
            "usage": usage,
    ...
    if capability_id:
        ...
        emit_progress(
            progress,
            "Defining the capability market",
            "Matching exact item classes, sources, procurement and aircraft platforms",
            42,
        )
        pack = runtime.call_tool("get_capability_market", arguments)
        trace = [
        ...
        emit_progress(
            progress,
            "Preparing the supplier view",
            "Comparing authorized sources, observed procurement and platform use",
            68,
        )
        response = client.responses.create(
            model=runtime.model,
            instructions=CAPABILITY_DISCOVERY_PROMPT,
    ...
    if item_query:
        ...
        if resolution.get("requires_identifier_confirmation"):
            requested_nsn = str(resolution.get("requested_nsn") or item_query)
            catalog_nsn = str(resolution.get("catalog_nsn") or "")
            match = (resolution.get("matches") or [{}])[0]
            niin = str(match.get("niin") or "")
            description = str(match.get("description") or "the referenced item")

            def display_nsn(value: str) -> str:
                if len(value) != 13:
                    return value
            ...
            return {
                "answer": (
                    f"I could not match **NSN {display_nsn(requested_nsn)}** as an exact "
                    f"current item. Its NIIN portion, **{niin}**, is listed under "
                    f"**NSN {display_nsn(catalog_nsn)}** for {description}.\n\n"
                    f"Please confirm the FSC, or ask me about **NIIN {niin}** to analyse the "
                    "current DLA item record."
                ),
    ...
    if platform_query:
        ...
        if resolved_platform:
            ...
            if is_platform_confirmation(latest_platform_question, resolved_platform):
                prior_question = next((message.content for message in reversed(request.messages[:-1]) if message.role == "user"), None)
                if prior_question:
                    latest_platform_question = prior_question
            answer_mode = (
            ...
            platform_input_items: List[Any] = [
                {
                    "role": message.role,
                    "content": message.content if message.role == "user" else message.content[:16000],
                }
                # Preserve the last two exchanges without resending an ever-growing report history.
